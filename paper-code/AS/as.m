...
load GNSS_20221110_20231110
data1=GNSS_20221110_20231110';
data1=data1;
...
load seed.mat
rng(s)
while ii<10000                                                                                                                                                                                                                            %??°??????é????????3000?¨???…???é????§???é????·???é??‘°????’????????
    ii=ii+1;
    input_dimensions=stationnum;
    size(data1);
    [all_weights, reverse_all_weights]=weights_align1(weight_a,weight_b,weight_c,size(data1,1));   
    ...
    while normal_start+samples-1 < sample_use-samples+samples    
        ...
        normal_sd_original(:,normal_start)=std(data1(:,myzone{1}),0,2); 
        for mys=1:1
            UU=data1(:, myzone{mys});
        ...
        normal_sd_y(normal_start)=normal_sd_y(normal_start) + std(test_flat_y(1,1:end));    %????—??????“??¨??????°??°?????‘é????§??????????–?é?”??????é?‰?
        reps=reps+1;
    end
    if mod(ii,100)==0      
        ii
...
% 保存所有的预测值
predicted_values = test_flat_y; % 预测值
% 保存到.mat文件
save('predicted_values.mat', 'predicted_values');
